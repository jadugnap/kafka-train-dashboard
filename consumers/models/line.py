...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "dashboard.faust.sanitized.stations": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "dashboard.producer.train." in message.topic(): # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic() == "DASHBOARD_SUMMARY_TURNSTILE": # Set the conditional to the KSQL Turnstile Summary Topic
