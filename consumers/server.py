...
def run_server():
    ...
    if topic_check.topic_exists("DASHBOARD_SUMMARY_TURNSTILE") is False:
        ...
        exit(1)
    if topic_check.topic_exists("dashboard.faust.sanitized.stations") is False:
        logger.fatal(
    ...
    consumers = [
        KafkaConsumer(
            "dashboard.producer.weather",
            weather_model.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
            "dashboard.faust.sanitized.stations",
            lines.process_message,
            offset_earliest=True,
            is_avro=False,
        ),
        KafkaConsumer(
            "^dashboard.producer.train.",
            lines.process_message,
            offset_earliest=True,
        ),
        KafkaConsumer(
