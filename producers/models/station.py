...
class Station(Producer):
    ...
    def __init__(self, station_id, name, color, direction_a=None, direction_b=None):
        ...
        # replicas
        topic_name = f"dashboard.producer.train.{station_name}.{station_id}" # TODO: Come up with a better topic name
        super().__init__(
            topic_name,
            key_schema=Station.key_schema,
            value_schema=Station.value_schema,
            num_partitions=1, # to revisit if necessary
            num_replicas=1, # only 1 broker
        )
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        try:
            ...
            Topic [{}]
            has error: [{}]
            '''.format(self.station_id, train.train_id, direction, self.color.name, train.status.name, prev_station_id, prev_direction, self.topic_name, e))
